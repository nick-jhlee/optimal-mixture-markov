...
def plot_panels(summary: pd.DataFrame, savepath: str = "results/ablation3_results.pdf", h_split: Tuple[Sequence[int], Sequence[int]] | None = None) -> None:
...
def main():
    ...
    df.to_csv("results/ablation3_results_unknownK_raw.csv", index=False)

    summary = summarize(df, alpha=0.05)
    summary.to_csv("results/ablation3_results_unknownK_summary.csv", index=False)

    # Plot with two-row layout (H_list_1 in row 1, H_list_2 in row 2)
    plot_panels(summary, savepath="results/ablation3_results.png", h_split=(H_list_1, H_list_2))
    plot_panels(summary, savepath="results/ablation3_results.pdf", h_split=(H_list_1, H_list_2))
